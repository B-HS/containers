...
즉 순서는 **터널을 열기 전에** 서버에서 `http://127.0.0.1:18080` 으로 접속해 owner 를 만들고, 그 뒤 노출한다. 이미 노출한 뒤라도 서버에서 로컬 주소로 접속하면 된다. 공개 화면에는 폼 대신 로컬에서 설정하라는 안내가 나온다.

이후 사용자는 초대로만 늘어난다. 계정을 초기 상태로 되돌리려면 [RUNBOOK](./RUNBOOK.md) §10 을 따른다.
