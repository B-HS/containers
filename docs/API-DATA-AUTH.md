...
- 모든 handler는 `withErrorHandling` 으로 감싸고, 그 **안에서** 인증한다. 세션·API key 겸용 경로는 `lib/authenticate-scope-or-role.ts` 의 `authenticateScopeOrRole({ headers, roles, scope })` 헬퍼가 `Authorization` 헤더 유무로 분기하고, 세션 전용 경로만 `authService.requireRole` 을 직접 호출한다. (`lib/with-auth.ts` 의 `withAuth`·`withAdmin`·`withApiToken` HOF 는 정의만 있고 라우트에서 쓰지 않는다. `withCapability` 는 존재하지 않는다.)
...
- **API key**: `authorization: Bearer ctk_...`. `Authorization` 헤더가 있으면 route가 API key 경로로 분기하고, 없으면 세션 경로로 간다. 키는 sha256 해시로만 저장되며 scope·만료·분당 rate limit(`API_KEY_RATE_LIMIT_PER_MINUTE`, 기본 120)을 적용한다. **API key 는 발급자의 비대화형 자격증명이다** — 키 보유자의 현재 role 이 세션과 동일한 role 게이트를 통과해야 하고(owner 전용 작업은 owner 의 키만), owner 전용 scope(`backup:restore`·`backup:write`·`maintenance:write`·`panel-setting:write`·`registry-credential:write`·`secret:write`·`system:prune`·`trusted-proxy:write`)는 발급·사용 모두 owner 에게만 허용된다. 키 경로의 mutating audit 에는 `apiKeyId` 가 detail 로 기록된다.

**세션 전용(API key 불가)**: exec(`POST /api/containers/:id/exec`, exec-tickets, WS), API 키 관리 자체(`/api/api-keys`), 계정·초대·bootstrap(`/api/auth/*`, `/api/session`, `/api/users`, `/api/invitations`, `/api/bootstrap/*`). — [acknowledge/0044](./acknowledge/0044-api-key-parity-and-egress-broker.md)
...
| `DELETE /api/artifacts/:artifactId`                                                                             | `artifact:write`                                    | owner·admin                                                |
| `POST /api/uploads/sessions`, `PUT .../chunks`, `POST .../finalize`                                             | `artifact:upload`                                   | owner·admin·operator                                       |
| `POST /api/artifacts/:artifactId/load`                                                                          | `image:load`                                        | owner·admin                                                |
| deployment 계열 조회 (manifests·stacks·releases)                                                                | `deployment:read`                                   | 전 역할                                                    |
| deployment 계열 변경 (manifests·stacks·releases CUD·rollback·preview)                                           | `deployment:write`                                  | owner·admin                                                |
| `GET /api/deployment-secrets`, `GET .../key-versions`                                                           | `secret:read`                                       | owner·admin                                                |
| `POST`·`DELETE /api/deployment-secrets`                                                                         | `secret:write`(owner)                               | owner·admin                                                |
| `POST /api/deployment-secrets/rotate`                                                                           | `secret:write`(owner)                               | owner                                                      |
| `GET /api/jobs`, `/api/jobs/:id`, `/:id/events`, `/backup-schedule`                                             | `job:read`                                          | owner·admin                                                |
| `POST /api/jobs/:id/cancel`                                                                                     | `job:write`                                         | owner·admin                                                |
| `GET /api/backups`                                                                                              | `backup:read`(owner)                                | owner                                                      |
| `POST /api/backups` / `DELETE /api/backups/:id`                                                                 | `backup:write`(owner)                               | owner                                                      |
| `POST /api/backups/:id/restore`                                                                                 | `backup:restore`(owner)                             | owner                                                      |
| engine 조회 (`/api/system/engine`, containers·logs·top·changes, images, networks, volumes, `/api/stream/*` SSE) | `engine:read`                                       | 전 역할                                                    |
| `GET /api/images/:id/removal-impact`, `GET /api/system/prune-preview`                                           | `engine:read`                                       | owner·admin                                                |
| `GET /api/control-plane/status`                                                                                 | `control-plane:read`                                | owner·admin                                                |
| `/api/nginx/*` 조회                                                                                             | `nginx:read`                                        | 전 역할                                                    |
| `/api/nginx/*` 변경 (routes CUD, `config/apply`)                                                                | `nginx:write`                                       | owner·admin                                                |
| `/api/traffic/*` 조회·live SSE                                                                                  | `traffic:read`                                      | 전 역할 (`/traffic/health` 는 owner·admin)                 |
| `POST /api/traffic/exports`, `GET .../download`                                                                 | `traffic:export`                                    | owner·admin                                                |
| `/api/audit`, `/api/audit/integrity`                                                                            | `audit:read`                                        | owner·admin·viewer·auditor / owner·admin·auditor           |
| `POST /api/containers`, `POST /:id/actions`, `POST /:id/wait`                                                   | `container:write`                                   | owner·admin (wait·일부 액션은 operator)                    |
| `POST /api/images/pull`, `POST /:id/tag`, `DELETE /api/images/:id`                                              | `image:write`                                       | owner·admin (force 삭제는 owner)                           |
| `POST`·`DELETE /api/networks/*`                                                                                 | `network:write`                                     | owner·admin                                                |
| `POST`·`DELETE /api/volumes/*`                                                                                  | `volume:write`                                      | owner·admin                                                |
| `GET /api/registry-credentials`                                                                                 | `registry-credential:read`                          | owner·admin                                                |
| `/api/registry-credentials` 변경                                                                                | `registry-credential:write`(owner)                  | owner                                                      |
| `POST /api/system/prune`                                                                                        | `system:prune`(owner)                               | owner                                                      |
| `GET /api/maintenance` / `POST /api/maintenance`                                                                | `maintenance:read` / `maintenance:write`(owner)     | 전 역할 / owner                                            |
| `/api/notification-destinations` 조회 / 변경·test                                                               | `notification:read` / `notification:write`          | owner·admin / owner·admin                                  |
| `GET /api/panel-settings` / `PUT`                                                                               | `panel-setting:read` / `panel-setting:write`(owner) | owner·admin / owner                                        |
| `GET /api/trusted-proxies` / `POST`·`DELETE`                                                                    | `trusted-proxy:read` / `trusted-proxy:write`(owner) | owner·admin / owner                                        |
| exec(`/api/containers/:id/exec`, exec-tickets, `/api/exec/ws/:ticket`)                                          | 불가                                                | owner만                                                    |
| `/api/api-keys` 조회 / 생성·폐기                                                                                | 불가                                                | owner·admin / owner·admin (owner 전용 scope 포함 시 owner) |
| `/api/auth/*`, `/api/session`, `/api/users`, `/api/invitations`, `/api/bootstrap/*`                             | 불가                                                | Better Auth 세션 (bootstrap 은 로컬 주소 전용)             |
