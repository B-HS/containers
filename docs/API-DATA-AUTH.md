...
| `DELETE /api/artifacts/:artifactId`                                                                             | `artifact:write`                                    | recent owner·admin                                                       |
...
| deployment 계열 변경 (manifests·stacks·releases CUD·rollback·preview)                                           | `deployment:write`                                  | recent owner·admin                                                       |
...
| `POST /api/deployment-secrets/rotate`                                                                           | `secret:write`(owner)                               | recent owner                                                             |
...
| `POST /api/backups` / `DELETE /api/backups/:id`                                                                 | `backup:write`(owner)                               | recent owner                                                             |
| `POST /api/backups/:id/restore`                                                                                 | `backup:restore`(owner)                             | recent owner                                                             |
...
| `/api/nginx/*` 변경 (routes CUD, `config/apply`)                                                                | `nginx:write`                                       | recent owner·admin                                                       |
...
| `POST /api/containers`, `POST /:id/actions`, `POST /:id/wait`                                                   | `container:write`                                   | recent owner·admin (wait·일부 액션은 operator)                           |
| `POST /api/images/pull`, `POST /:id/tag`, `DELETE /api/images/:id`                                              | `image:write`                                       | recent owner·admin (force 삭제는 owner)                                  |
| `POST`·`DELETE /api/networks/*`                                                                                 | `network:write`                                     | recent owner·admin                                                       |
| `POST`·`DELETE /api/volumes/*`                                                                                  | `volume:write`                                      | recent owner·admin                                                       |
| `GET /api/registry-credentials`                                                                                 | `registry-credential:read`                          | owner·admin                                                              |
| `/api/registry-credentials` 변경                                                                                | `registry-credential:write`(owner)                  | recent owner                                                             |
| `POST /api/system/prune`                                                                                        | `system:prune`(owner)                               | recent owner                                                             |
| `GET /api/maintenance` / `POST /api/maintenance`                                                                | `maintenance:read` / `maintenance:write`(owner)     | 전 역할 / recent owner                                                   |
| `/api/notification-destinations` 조회 / 변경·test                                                               | `notification:read` / `notification:write`          | owner·admin / recent owner·admin                                         |
| `GET /api/panel-settings` / `PUT`                                                                               | `panel-setting:read` / `panel-setting:write`(owner) | owner·admin / recent owner                                               |
| `GET /api/trusted-proxies` / `POST`·`DELETE`                                                                    | `trusted-proxy:read` / `trusted-proxy:write`(owner) | owner·admin / recent owner                                               |
| exec(`/api/containers/:id/exec`, exec-tickets, `/api/exec/ws/:ticket`)                                          | 불가                                                | recent owner만                                                           |
| `/api/api-keys` 조회 / 생성·폐기                                                                                | 불가                                                | owner·admin / recent owner·admin (owner 전용 scope 포함 시 recent owner) |
