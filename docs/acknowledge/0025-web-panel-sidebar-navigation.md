...
근거: 사용자 지시 — "프로젝트를 정확하게 읽고 현재 웹패널의 내용들을 왼쪽 사이드바에 메뉴로 해서 하나씩 분류. 지금 한 페이지에 싹 다 들어가 있어서 설정이 매우 불편함. UI/UX 를 완벽하게 고려해서 정확하게 분리해서 구현". 분리 결과: `apps/web/src/app/[locale]/(panel)/`, [UI-UX.md](../UI-UX.md)
