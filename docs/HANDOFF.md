...
상세는 [acknowledge/](./acknowledge/) (최신 **0043**).
...
2026-08-06 기준으로 사용자 확인을 기다리는 항목은 없다. 아래는 **닫힌 결정과 그 결과**이므로, 새 세션은 이걸 다시 묻지 않는다.

| 항목                        | 결정                       | 결과·근거                                                                                                                                                        |
| --------------------------- | -------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 아티팩트·manifest·스택 삭제 | A안(이력 유지 + 파일 회수) | 구현·실측 완료 → [bug](./bug/2026-08-06-loaded-artifacts-cannot-be-deleted.md)                                                                                   |
| 실운영 owner                | bootstrap 으로 생성        | 완료. seed 계정 없음                                                                                                                                             |
| 공개 주소                   | `https://hyuns.uk`         | 패널 설정에 등록, nginx `server_name` 반영 확인                                                                                                                  |
| control DB 고아 참조        | 이력 귀속을 nullable 로    | migration `0026` + 스크립트 FK 강제 → [bug](./bug/2026-08-06-reset-accounts-broke-referential-integrity.md)                                                      |
| 오프박스 백업               | **하지 않는다**            | 둘 곳이 없다는 사용자 판단. 대가는 "디스크가 죽으면 데이터와 백업이 함께 사라진다" 이며 이는 알고 감수하는 위험이다. 저장 위치가 생기면 그때 붙인다              |
| Cloudflare Access           | **걸지 않는다**            | 로그인 화면 도달은 상관없다는 사용자 판단. 그 전제 위의 방어가 계정 잠금·rate limit·짧은 세션·CSP nonce 다                                                       |
| `CF-Connecting-IP` 위조     | **손대지 않는다**          | 로컬 권한자 한정이고 실익이 로그 오염뿐이다. 로그인 무차별 대입은 IP 가 아니라 계정 기준 잠금이 막는다                                                           |
| `containers-dr-*` 이미지    | 삭제함                     | 재해복구 드릴 산출물 5개(약 770MB). 쓰는 컨테이너가 없고 필요하면 다시 빌드한다. 드릴 기록은 `quality-assurance/2026-08-05-disaster-recovery-drill.md` 에 남는다 |
| HSTS `preload`              | **넣지 않는다(현행 유지)** | 등재는 쉬우나 취소가 느리다. 개인 control plane 에 그 비가역성을 지불할 이유가 없다. `max-age`·`includeSubDomains` 는 이미 적용돼 있다                           |

**남은 것은 "결정"이 아니라 "아직 해보지 않은 검증"뿐이다** — §2.1 과 §8 을 본다.
...
1. 이 문서 §2.1(실운영 판정)·§6(닫힌 결정)·§7(환경) 을 읽는다. **다시 물을 것이 없다** — §6 은 재질문 방지용 기록이다.
