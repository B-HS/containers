...
## compose 스택 화면 (2026-08-06)

`/deployments/stacks` 는 compose 파일을 manifest 로 바꿔 스택 단위로 배포하는 화면이다. 기존 `/deployments`(단일 manifest)와 나란히 배포 섹션에 둔다.

- **미리보기가 먼저다.** 등록 전에 변환 결과(서비스별 manifest 이름·digest·포트·라우트, 위상 순서, 무시한 키)를 보여준다. 미리보기는 서버에 아무것도 저장하지 않는다.
- 내부 서비스(`route: null`)는 도메인 대신 "내부 서비스" 배지로 구분한다.
- compose 원문은 파일 선택 또는 직접 입력 둘 다 받는다. 파일은 텍스트로 읽어 textarea 를 채우므로 업로드 세션을 쓰지 않는다.
- 스택 상세는 최신 배포의 상태 배지(배포 중·정상·실패·되돌림)와 실패 코드, 서비스·릴리스 개수를 보여준다. 배포 중에는 시작 버튼이 잠긴다.
- **거부 사유의 상세(어느 서비스의 어느 키인지)는 아직 토스트에 실리지 않는다.** 서버는 `details.rejections` 로 내려보내는데 웹의 에러 파서가 버린다 — 4.1 에서 정리한다.
