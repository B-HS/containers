...
- [x] **5.1 nginx 편집기 컨테이너 인지 + 두 화면 역할 안내** — upstream 주소 입력에 실행 중 컨테이너 `이름:포트` 제안(datalist), 두 nginx 화면에 역할 설명과 상호 링크. 실측: `/nginx` 안내가 `/ko/nginx/routes` 로 링크
- [x] **5.2 접근성 4건** — directive 값 입력에 `aria-label`(지시어 이름)과 삭제 버튼 이름, 표 스크롤 영역 `tabIndex=0`+포커스 링, 확인 입력 불일치 안내 `aria-live`. 컨트롤 없는 `<Label>` 2건은 표·배지 묶음의 제목이라 그대로 둔다
- [x] **5.3 삭제 확인 다이얼로그 피드백** — 입력해야 할 값을 자르지 않고 `break-all`+`title` 로 전부 보이고, 불일치 시 `aria-invalid`와 사유 문구를 띄운다(이전에는 버튼만 비활성)
