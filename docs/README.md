...
29. [quality-assurance/2026-08-05-panel-ux-audit.md](./quality-assurance/2026-08-05-panel-ux-audit.md) — 패널 UX 감사(원본 64건 → 검증 44건 → 원인 12개)와 배포 런타임 제약 실측
30. `ci-examples/` — CI 워크플로 예제(빌드 CI 3종 + API key 기반 배포 워크플로)
31. `acknowledge/` — 번호 순 확정 결정 기록 (최신: 0036 공개 주소 단일 출처·종료 드레인)
32. `history/` — 세션별 작업 이력 (시간순, 현재 상태가 아님)
