...
export const createRegistryCredentialRoute = ({ registryCredentialService }: RegistryCredentialRouteDependencies) => {
    ...
    route.post(
        ...
        withErrorHandling(async (context) => context.json(await registryCredentialService.upsert(undefined, context.req.valid('json' as never)), 201)),
    )
    route.post(
        ...
        withErrorHandling(async (context) =>
            context.json(await registryCredentialService.upsert((context.req.valid('param' as never) as { credentialId: string }).credentialId, context.req.valid('json' as never)), 200),
        ),
    )
    route.delete(
        '/registry-credentials/:credentialId',
        ...
        withErrorHandling(async (context) =>
            context.json(await registryCredentialService.remove((context.req.valid('param' as never) as { credentialId: string }).credentialId, context.req.valid('json' as never)), 200),
        ),
    )
