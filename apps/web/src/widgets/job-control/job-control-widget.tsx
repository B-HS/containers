...
import { operationJobSchema, type BackupSchedule, type OperationJob } from '@containers/contracts/operation-job'
...
const cancelResponseSchema = z.object({ data: operationJobSchema, success: z.literal(true) })

export const JobControlWidget: FC<JobControlWidgetProps> = ({ jobs: initialJobs, labels, maintenance, schedule }) => {
    ...
    const cancel = async (jobId: string) => {
        ...
        try {
            ...
            const cancelled = cancelResponseSchema.parse(body).data
    ...
    return (
        <section className="mt-px min-w-0 overflow-hidden bg-card" aria-labelledby="job-control-title">
            <header className="flex items-center justify-between p-3">
                <h2 id="job-control-title" className="text-sm font-semibold">
                    {labels.title}
                </h2>
