...
describe('applyPanelHostname', () => {
    test('패널 server 블록에만 hostname 을 추가한다', () => {
        const next = applyPanelHostname(CONFIG, { add: 'hyuns.uk' })

        expect(next).not.toBeNull()
        expect(readPanelServerNames(next ?? '')).toEqual(['panel.containers.local', 'localhost', '127.0.0.1', 'hyuns.uk'])
    ...
    test('제거만 하면 내장 이름은 남는다', () => {
        const added = applyPanelHostname(CONFIG, { add: 'hyuns.uk' }) ?? ''
        const removed = applyPanelHostname(added, { remove: 'hyuns.uk' })
    ...
    test('패널 블록이 없으면 null 을 돌려준다', () => {
        expect(applyPanelHostname('events {} http { server { server_name other.local; } }', { add: 'hyuns.uk' })).toBeNull()
    ...
    test('다른 지시어와 들여쓰기를 보존한다', () => {
        ...
        expect(next).toContain('        server_name panel.containers.local localhost 127.0.0.1 hyuns.uk;')
